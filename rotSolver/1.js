...
function compoundWord(arr){
    let arrToObject = arr.reduce((a, v) => ({ ...a, [v]: v}), {})
    let compoundArr = []
    console.log(arrToObject)
    for(let i = 0; i < arr.length; i++){
        for(let j = 0; j < arr[i].length; j++){
            let firstWord = arr[i].slice(0,j+1)
            let secondWord = arr[i].slice(j+1)
            
            if(firstWord in arrToObject && secondWord in arrToObject){
                compoundArr.push(arr[i])
            }
        }
    }
    return compoundArr
}
console.log(compoundWord(['SNOWMAN', "SNOW", 'MAN', 'are', 'all', 'of', 'the', 'compound', 'words', 'These', 'are', 'words']))
